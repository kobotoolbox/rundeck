...
echo ENV:$ENV
echo ENV_FRONTEND:$ENV_FRONTEND
...
if [[ ${TEST_DOCKER_NGINX} == "true" ]] && [[ ${TEST_DOCKER_KC} == "true" ]] && [[ ${TEST_DOCKER_KPI} == "true" ]] && [[ ${TEST_DOCKER_EE} == "true" ]]; then
    ...
    if  [[ ${ENV_FRONTEND} == "frontend_primary_only" ]]; then
    ...
    elif [[ ${ENV_FRONTEND} == "frontend_all" ]]; then
        IP_FRONTEND_ALL=$($AWS ec2 describe-instances \
            --region ${EC2_REGION} \
        ...
        for line in $IP_FRONTEND_ALL; do
            # Get DNS  
